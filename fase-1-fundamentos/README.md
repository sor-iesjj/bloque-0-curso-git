# FASE 1 — Git Fundamentals

**Duración:** 4 h presenciales

Marko arranca: instala Git, configura su identidad, crea su primera bóveda, hace commits, sube a GitHub. De cero a tener su trabajo en la nube.

> [!important] 🔀 Si estás haciendo también los PRERREQUISITOS de SOR (lo normal)
> Este curso va **en paralelo** con el bloque de prerrequisitos, y **el Nivel 1 se solapa con él**: las Fases 0.1, 0.2.1, 0.2.2 y 0.3 hacen exactamente lo mismo que los ejercicios `EJ-01-01-01` a `EJ-01-01-05`.
>
> **No lo hagas dos veces.** Empieza por el **[`EJ-01-01-00` — Marko comprueba que trae el equipo listo](EJ-01-01-00.md)**: en 20 minutos verificas que lo tienes todo y **te saltas los cinco**. Si algo falla, ese mismo ejercicio te dice a qué fase de prerrequisitos ir.
>
> Los cinco del Nivel 1 se quedan como **material de consulta** y para quien llegue **sin** haber hecho prerrequisitos.

---

## Índice de ejercicios

### 🔵 N0 — Diagnóstico (1 ejercicio) · **EMPIEZA AQUÍ**

- [`EJ-01-01-00`](EJ-01-01-00.md) — **Marko comprueba que trae el equipo listo** ← sustituye a los cinco de abajo si ya has hecho la Fase 0.3

### 🟢 N1 — Mínimo (5 ejercicios) · *solo si NO has hecho prerrequisitos*

- [`EJ-01-01-01`](EJ-01-01-01.md) — Marko instala Git por primera vez
- [`EJ-01-01-02`](EJ-01-01-02.md) — Marko se identifica ante Git
- [`EJ-01-01-03`](EJ-01-01-03.md) — Marko crea su bóveda y su primer repositorio de manuales
- [`EJ-01-01-04`](EJ-01-01-04.md) — Marko hace su primer commit (manual de Ubuntu Server)
- [`EJ-01-01-05`](EJ-01-01-05.md) — Marko publica su repositorio de manuales en GitHub

### 🟡 N2 — Notable (6 ejercicios)

- [`EJ-01-02-01`](EJ-01-02-01.md) — Marko amplía el manual y registra el cambio
- [`EJ-01-02-02`](EJ-01-02-02.md) — Marko se equivoca y deshace el cambio antes de commitear
- [`EJ-01-02-03`](EJ-01-02-03.md) — Marko crea su .gitignore (no subir basura)
- [`EJ-01-02-04`](EJ-01-02-04.md) — Marko navega su histórico (`git log`)
- [`EJ-01-02-05`](EJ-01-02-05.md) — Marko clona el repo de Carlos para consultarlo
- [`EJ-01-02-06`](EJ-01-02-06.md) — Marko sincroniza cambios remotos (`git pull`)

### 🔴 N3 — Sobresaliente (4 ejercicios)

- [`EJ-01-03-01`](EJ-01-03-01.md) — Marko enmienda su último commit
- [`EJ-01-03-02`](EJ-01-03-02.md) — Marko crea alias para no escribir tanto
- [`EJ-01-03-03`](EJ-01-03-03.md) — Marko verifica integridad con `git fsck` y limpia con `git gc`
- [`EJ-01-03-04`](EJ-01-03-04.md) — Marko firma sus commits para garantizar autoría

---

### 🏁 Cierre

- [`Cierre_Fase_1`](Cierre_Fase_1.md) — repaso, preguntas y verificación **antes de pasar a la Fase 2**

---

[← Volver al índice del curso](../README.md)
